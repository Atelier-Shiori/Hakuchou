...
+ (NSDictionary *)retrieveMetaDataWithID:(int)titleid withMetaData:(NSArray *)metadata {
    NSArray *filtered = [metadata filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"id == %@", @(titleid).stringValue]];
    if (filtered.count > 0) {
        return filtered[0];
    }
    return nil;
